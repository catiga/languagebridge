...
func CreateStageGoal(c *gin.Context) {
	...
	m := model.UserPlanOverview{
		...
		Status:         common.StudyPlannerOverviewStatusCreate,
		GoalPeriodType: req.GoalPeriodType,
		TargetLevel:    req.TargetLevel,
		InitLevel:      req.InitLevel,
	}
	db.Save(&m)
	...
	res.Data = map[string]interface{}{
		...
		"status":           m.Status,
		"add_time":         m.AddTime,
		"goal_period_type": m.GoalPeriodType,
		"target_level":     m.TargetLevel,
		"init_level":       m.InitLevel,
	}
...
func PullStageGoal(c *gin.Context) {
	...
	for _, v := range overviewList {
		...
		retOverviewList = append(retOverviewList, UserPlanOverviewResponse{
			...
			EndDate:        v.EndDate,
			Tasks:          taskDataSub,
			GoalPeriodType: v.GoalPeriodType,
...
func ViewStageGoal(c *gin.Context) {
	res := common.Response{}
	res.Timestamp = time.Now().Unix()
	...
	var overviewId = uint64(0)
	if exist {
		overviewId, _ = strconv.ParseUint(overviewIdStr, 10, 64)
	}
	...
	var sql = "user_id = ? and flag != ? and id = ?"
	var params = []interface{}{userID, -1, overviewId}
	err = db.Model(&model.UserPlanOverview{}).Where(sql, params...).First(&overview).Error
	if err != nil {
		log.Error(err)
	}
	...
	err = db.Model(&model.UserPlanSchedule{}).Where("overview_id = ? and flag != ?", overview.ID, -1).Order(("start_time DESC")).Find(&taskList).Error
	if err != nil {
		log.Error(err)
	}
	...
	var taskDataSub []UserPlanScheduleResponse = make([]UserPlanScheduleResponse, 0)
	if len(taskList) > 0 {
		for _, s := range taskList {
			taskDataSub = append(taskDataSub, UserPlanScheduleResponse{
				ID:        s.ID,
	...
	retOverviewList = append(retOverviewList, UserPlanOverviewResponse{
		...
		Goal:           overview.Goal,
		AddTime:        overview.AddTime,
		StartDate:      overview.StartDate,
		EndDate:        overview.EndDate,
		Tasks:          taskDataSub,
		GoalPeriodType: overview.GoalPeriodType,
		TargetLevel:    overview.TargetLevel,
		InitLevel:      overview.InitLevel,
