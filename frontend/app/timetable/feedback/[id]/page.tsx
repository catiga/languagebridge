            ...
            {feedbacks.map(fb => (
              <div key={fb.id} className="bg-gray-50 rounded-lg p-4 shadow flex flex-col gap-1">
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2 py-0.5 rounded text-xs font-bold bg-yellow-100 text-yellow-700">{fb.author || 'Feedback'}</span>
                  <span className="text-xs text-gray-500">{fb.add_time}</span>
                  {typeof fb.score === 'number' && (
                    <span className="ml-2 text-xs text-orange-500 font-bold">Score: {fb.score}</span>
                  )}
                </div>
                <div className="text-gray-800 text-base whitespace-pre-line">{fb.content}</div>
              </div>
            ))}
          </div>
        )
