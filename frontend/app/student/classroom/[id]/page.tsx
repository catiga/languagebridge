...
export default function MeetPage() {
  ...
  useEffect(() => {
    ...
    const fetchMeetingInfo = async () => {
      try {
        const res = await apiClient.get('/spwapi/student/auth/course/meeting/fetch', { btid: id }) as any;
  ...
  const handleReadyToClose = async () => {
    try {
      // 通知后端用户已结束会议
      await apiClient.get('/spwapi/student/auth/course/meeting/end', { btid: id });
    } catch (e) {
  ...
  const fetchNotes = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get('/spwapi/student/auth/course/meeting/note/fetch', { btid: id }) as any;
  ...
  const handleAddNote = async () => {
    ...
    try {
      const res = await apiClient.post('/spwapi/student/auth/course/meeting/note/add', { note: content, btid: Number(id) }) as any;
      if (res && res.code === 0) {
        setNoteInput('');
        setNoteError('');
        fetchNotes();
