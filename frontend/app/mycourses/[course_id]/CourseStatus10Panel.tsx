...
export default function CourseStatus10Panel({ course, params }: { course: any, params: { course_id: string } }) {
  ...
  const handleRefund = async () => {
    ...
    try {
      // TODO: 替换为真实退款接口
      // const res: any = await apiClient.get('/spwapi/auth/course/refund', { uc_id: course.uc_id });
      // if (res && res.code === 0) {
      //   toast.success('Refund requested successfully!');
      //   setTimeout(() => router.push('/profile/courses'), 1200);
      // } else {
      //   toast.error(res?.msg || 'Refund failed');
      // }
      toast.info('Refund request sent (mock)');
    } catch (e: any) {
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-md p-0 mt-8 border-2 border-green-400 relative overflow-hidden">
      {/* 顶部绿色渐变条 */}
      <div className="h-2 w-full bg-gradient-to-r from-green-400 via-green-300 to-green-500 absolute top-0 left-0" />
      <div className="p-8 pt-6">
        {/* 顶部提示语和状态徽章 */}
        <div className="flex items-center justify-between mb-4">
          <span className="text-green-700 font-semibold text-base">Your course is ongoing. Enjoy your learning journey!</span>
          <span className="inline-flex items-center px-3 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700 border border-green-300 ml-2">Active</span>
        </div>
        {/* 课程图片和基本信息 */}
        ...
        <div className="mb-6 p-6 rounded-xl shadow bg-white border border-gray-200">
          ...
          {teacherLoading ? (
            <div className="py-2 text-green-600">Loading teacher info...</div>
          ) : teacher ? (
            <div className="flex items-center gap-4 mb-4">
              <img src={teacher.avatar || '/default-avatar.svg'} alt={teacher.name} className="w-16 h-16 rounded-full object-cover border-2 border-green-300" />
              <div>
        ...
        <div className="flex justify-end gap-4 mt-6">
          <button
            className={`px-4 py-2 rounded bg-green-600 text-white hover:bg-green-700 transition font-semibold ${refundLoading ? 'opacity-60 cursor-not-allowed' : ''}`}
            onClick={handleRefund}
            disabled={refundLoading}
          >
            {refundLoading ? 'Requesting...' : 'Request Refund'}
          </button>
        </div>
